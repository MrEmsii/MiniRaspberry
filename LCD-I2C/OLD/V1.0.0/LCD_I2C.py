import API_LCD_I2C
import time, w1thermsensor, psutil, socket
from gpiozero import CPUTemperature
from subprocess import check_output

mylcd = API_LCD_I2C.lcd()
sensor = w1thermsensor.W1ThermSensor()

status = 3
start_time = float(time.time()) + 5
temp_time = float(time.time()) + 10
miesiace = ["Stycznia", 'Lutego', 'Marca', 'Kwietnia', 'Maja', 'Czerwca', 'Lipca', 'Sierpnia', 'Wrzesnia', 'Pazdziernika', 'Listopada', 'Grudnia']
ost_status = status

def zegar():
	godzina = time.strftime("%H:%M:%S")
	data = time.strftime("%d") + " " + miesiace[int(time.strftime("%m")) - 1]
	rok = str(time.strftime("%Y"))
	
	mylcd.lcd_display_string(godzina, 2, int((20 - len(godzina))/2))
	mylcd.lcd_display_string(data, 3, int((20 - len(data))/2))
	mylcd.lcd_display_string(rok, 4, int((20 - len(rok))/2))

def termometr():
	temp_1 = "Temp Pokoj = " + str(float(sensor.get_temperature() - 4.5))[0:4] + "" + chr(223) + "C"
	temp_2 = "Temp RasPI = " + str(CPUTemperature().temperature)[0:4] + "" + chr(223) + "C"
	mylcd.lcd_display_string(str(temp_1), 2, int((20 - len(str(temp_1)))/2))
	mylcd.lcd_display_string(str(temp_2), 3, int((20 - len(str(temp_2)))/2))

def get_ip():
	cmd = str(check_output("hostname -I | cut -d\' \' -f1", shell=True).decode("utf-8").strip())
	if len(cmd) > 5 and len(cmd) < 16:
		return cmd
	else:
		return "No IP"

def stats():
	cpu_proc = "  CPU= " + str(psutil.cpu_percent(interval=0.5)) + "%  "
	RAM_proc = "  RAM= " + str(psutil.virtual_memory().percent) + "%  "
	disk_proc = "  DISK= " + str(psutil.disk_usage('/').percent) + "%  "
	print(get_ip())
	mylcd.lcd_display_string(str(get_ip()), 1, int((20 - len(str(get_ip())))/2))
	mylcd.lcd_display_string(str(cpu_proc), 2, 1)
	mylcd.lcd_display_string(str(disk_proc), 3, 0)
	mylcd.lcd_display_string(str(RAM_proc), 4, 1)

def timer():
	global start_time, status, ost_status

	if float(time.time()) >= start_time:
		start_time = float(time.time())	+ 5
		status = status + 1	
	if status >= 4:
		status = 1
	if status != ost_status:
		ost_status = status
		mylcd.lcd_clear()

def main():
	global status,start_time
	
	while True:
		save()
		timer()
		if status == 1:
			zegar()
			
		elif status == 2:
			termometr()
			
		elif status == 3:
			stats()
			
		elif status == 4:
			mylcd.lcd_display_string(sey(), 3, int((20 - len(sey()))/2))			
		time.sleep(0.5)
			
def save():
	global temp_time
	if float(time.time()) >= temp_time:
		temp_time = float(time.time())	+ 300
		f = open("/samba/python/Temperatura.txt", "a")
		f.write(time.strftime("%d.%m.%Y %H:%M:%S") + " " + str(float(sensor.get_temperature() - 4.5))[0:4]+ "" + chr(176) + "C" + "\n")
		
try:
	main()
except SyntaxError:
	print("Ups")
